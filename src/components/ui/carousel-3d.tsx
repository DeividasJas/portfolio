...
const Slide = ({ slide, index, current, handleSlideClick, onButtonClick }: SlideProps) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      <li
        ref={slideRef}
        className="relative z-10 mx-[4vmin] flex h-[70vmin] w-[70vmin] flex-1 flex-col items-center justify-center text-center text-white opacity-100 transition-all duration-300 ease-in-out lg:h-[50vmin] lg:w-[70vmin]"
...
export function Carousel3D({ slides, onButtonClick }: Carousel3DProps) {
  ...
  return (
    <div className="relative mx-auto h-[70vmin] w-[70vmin] lg:h-[50vmin]" aria-labelledby={`carousel-heading-${id}`}>
      <ul
      ...
      >
        {slides.map((slide, index) => (
          <Slide key={index} slide={slide} index={index} current={current} handleSlideClick={handleSlideClick} onButtonClick={onButtonClick} />
        ))}
      </ul>
